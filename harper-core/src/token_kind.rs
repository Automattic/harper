...
use crate::{LexemeMetadata, Number, Punctuation, Quote, TokenKind::Word};

/// Generate wrapper code to pass a function call to the inner [`LexemeMetadata`],  
/// if the token is indeed a word, while also emitting method-level documentation.
macro_rules! delegate_to_metadata {
    ($($method:ident),* $(,)?) => {
        $(
            #[doc = concat!(
                "Delegates to [`LexemeMetadata::",
                stringify!($method),
...
impl TokenKind {
    // Lexeme metadata delegation methods grouped by part of speech
    ...
    // Lexeme metadata delegation methods not generated by macro
