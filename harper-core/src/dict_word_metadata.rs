...
pub struct DictWordMetadata {
    ...
    /// Generated by a probability-aware POS tagger alongside [`Self::pos_tag`]: the
    /// set of *plausible* tags (the argmax plus close runner-ups). Lets linters
    /// match the reading they need on a genuine homograph instead of being
    /// defeated by a hair-thin argmax. Runtime-only — never serialized into the
    /// prebuilt dictionary; empty unless a probability-aware tagger populated it.
    #[serde(skip)]
    pub pos_tag_topk: SmallVec<[UPOS; 4]>,
}
...
impl DictWordMetadata {
    /// Whether `pos` is a plausible reading of this token for the tagger — true
    /// if it is the argmax [`Self::pos_tag`] or appears in the probability-aware
    /// [`Self::pos_tag_topk`] runner-up set. Probability-aware linters use this
    /// instead of `pos_tag == Some(pos)` so a genuine homograph (e.g. "books"
    /// tagged VERB with NOUN a close second) still matches the reading a rule needs.
    pub fn could_be_pos(&self, pos: UPOS) -> bool {
        self.pos_tag == Some(pos) || self.pos_tag_topk.contains(&pos)
    }
