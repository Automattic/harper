    ...
    for i, line in enumerate(lines):
        ...
        if entry["word"].lower() == noun_word.lower():
            ...
            if is_noun and compound_flag not in flags:
                ...
                modified = True
                print(
                    f"  Added {compound_flag} flag to: {entry['word']} (flags: {flags} -> {new_flags})"
                )
                break
    ...
    for i, line in enumerate(lines):
        ...
        if word_lower in common_nouns:
            ...
            if is_noun and target_flag not in flags:
                ...
                modified_count += 1
                print(
                    f"  Added {target_flag} flag to: {entry['word']} (flags: {flags} -> {new_flags})"
                )
...
def analyze_nouns_with_compound_flags(dict_path):
    ...
    print(
        f"  Percentage: {(with_compound_flags / total_nouns * 100):.1f}%"
        if total_nouns > 0
        else "  Percentage: 0%"
    )
...
def main():
    ...
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze nouns with compound flags"
    )
    parser.add_argument(
        "--add-common", action="store_true", help="Add compound flags to common nouns"
    )
    parser.add_argument("--word", type=str, help="Add compound flag to a specific noun")
    parser.add_argument(
        "--flag", type=str, default="h", help="Compound flag to add (h, i, k, l, m, o)"
    )
