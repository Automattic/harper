...
mod tests {
    ...
    create_test_pool!(ItsPossessive, ItsPossessive, ItsPossessive::default());
    ...
    fn paris_is_known_for_its_architecture() {
        assert_lint_count("Paris is known for it's architecture.", test_linter(), 1);
    }
    ...
    fn device_reached_its_100th_cycle() {
        assert_lint_count("The device reached it's 100th cycle.", test_linter(), 1);
    }

    #[test]
    fn oddly_its_wheels_misaligned() {
        assert_lint_count("Oddly, it's wheels were misaligned.", test_linter(), 1);
    }

    #[test]
    fn leaking_oil_constant_issue() {
        assert_lint_count("It's leaking oil constantly.", test_linter(), 0);
    }
    ...
    #[test]
    fn second_attempt() {
        assert_lint_count("He failed it's 2nd attempt.", test_linter(), 1);
    }

    #[test]
    fn third_iteration() {
        assert_lint_count("The program finished it's 3rd iteration.", test_linter(), 1);
    }

    #[test]
    fn tenth_milestone() {
        assert_lint_count("They reached it's 10th milestone.", test_linter(), 1);
    }

    #[test]
    fn seventh_chapter() {
        assert_lint_count("The novel lost it's 7th chapter.", test_linter(), 1);
    }

    #[test]
    fn fifth_version() {
        assert_lint_count("Software updated to it's 5th version.", test_linter(), 1);
    }

    #[test]
    fn eighth_floor() {
        assert_lint_count("Elevator stopped at it's 8th floor.", test_linter(), 1);
    }

    #[test]
    fn twelfth_episode() {
        assert_lint_count("Series ended it's 12th episode.", test_linter(), 1);
    }

    #[test]
    fn fourth_draft() {
        assert_lint_count("He completed it's 4th draft.", test_linter(), 1);
    }

    #[test]
    fn ninth_revision() {
        assert_lint_count("The report saved it's 9th revision.", test_linter(), 1);
    }

    #[test]
    fn allows_hard_to_tell() {
        assert_no_lints("It's hard to tell from here.", test_linter());
    }

    #[test]
    fn allows_illegible() {
        assert_no_lints("When you write in cursive, its illegible", test_linter());
    }
    ...
    #[test]
    fn allow_issue_1658() {
        assert_no_lints("It's kind of a nuisance, but it will work.", test_linter());
    }
    ...
    fn dont_flag_issue_1722_its_big_enough() {
        assert_no_lints("It's big enough.", test_linter());
    }

    #[test]
    fn allows_its_awesome() {
        assert_no_lints("It's awesome.", test_linter());
    }
