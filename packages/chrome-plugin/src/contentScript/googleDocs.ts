...
const GOOGLE_DOCS_IGNORED_LAYOUT_REASONS = new Set([
  "scroll",
  "wheel",
  "key-scroll",
]);
...
export function createGoogleDocsBridgeSync(
  fw: LintFramework,
): () => Promise<void> {
  ...
  function isStandaloneListMarker(text: string): boolean {
    return /^((\d+|[a-zA-Z]|[ivxlcdmIVXLCDM]+)[.)]|[-+*•◦▪‣●☐☑☒])$/u.test(
      text.trim(),
    );
  }
  ...
  function createCloneSpan(
    segment: GoogleDocsRectSegment,
    text: string,
  ): HTMLSpanElement {
  ...
  function segmentsShareVisualLine(
    ...
    return (
      rectSharesGoogleDocsLineBand(
        left.rect,
        createGoogleDocsLineBand(right.rect),
      ) ||
      rectSharesGoogleDocsLineBand(
        right.rect,
        createGoogleDocsLineBand(left.rect),
      )
    );
  }
  ...
  function normalizeLogicalGap(
    ...
    const startsNewTableColumn =
      ...
      rectSharesGoogleDocsLineBand(
        currentRect,
        createGoogleDocsLineBand(nextRect),
      ) &&
    ...
    const shouldKeepParagraphBreak =
      ...
      !isStandaloneListMarker(nextText);

    return skippedText.replace(
      /\n+/gu,
      shouldKeepParagraphBreak ? "\n\n" : "\n",
    );
  }
  ...
  function computeSignature(
    ...
    hash = addSignatureToken(
      hash,
      `${editor.scrollTop}:${editor.scrollLeft}:${segments.length}`,
    );
  ...
  function buildLogicalSnapshot(
    ...
    for (let index = 0; index < segments.length; index += 1) {
      ...
      const sharesVisualLine: boolean =
        ...
        rectSharesGoogleDocsLineBand(segment.rect, currentLineBand);
    ...
    for (const segment of segments) {
      ...
      if (startsNewLine && currentLineBand != null) {
        ...
        const paragraphBreakThreshold = Math.max(
          ...
          Math.min(currentLineBand.height, segment.rect.height) *
            GOOGLE_DOCS_PARAGRAPH_BREAK_RATIO,
        );
  ...
  function buildSnapshot(editor: HTMLElement): GoogleDocsCloneSnapshot {
    ...
    return (
      buildLogicalSnapshot(editor, segments, logicalText) ??
      buildRectSnapshot(editor, segments)
    );
  }

  function applySnapshot(
    target: HTMLElement,
    snapshot: GoogleDocsCloneSnapshot,
  ): boolean {
    if (
      snapshot.signature === lastCloneSignature &&
    ...
    target.setAttribute(
      "data-harper-gdocs-segments",
      String(snapshot.segmentCount),
    );
