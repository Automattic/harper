...
| `language`           | `{"English": "British"}`, `{"German": "Standard"}`, … | `{"English": "American"}` | Set the language and dialect Harper should expect. Overrides `dialect` when both are set. Prose files (Markdown, plain text, …) with enough text are detected automatically. |
