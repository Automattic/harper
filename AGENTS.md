...
- First-time/setup dependency path: `just pull-dep-source build-harper-deps`. This clones `automattic/harper` into root `harper/` and builds local `harper.js`/`harper-editor` packages used by `package.json`.
- Dev app: `just dev` from repo root. It runs `cargo tauri dev`; Tauri then runs `pnpm dev` on port `1420`.
- Frontend checks: `pnpm check`.
- Frontend build only: `pnpm build`.
- Rust checks from `src-tauri`: `cargo fmt --check`, `cargo check`, and `cargo test`.
- Rust formatting/fix loop from `src-tauri`: `cargo fmt && cargo check && cargo test`.
- Bundle builds match CI: `just build-linux` or `just build-macos`.
- `just build-linux` builds deb/rpm/appimage bundles.
- `just build-macos` builds app/dmg bundles.
...
- The editor UI is currently `src/routes/+page.svelte`, using local file dependencies from `./harper/packages/harper.js` and `./harper/packages/harper-editor`.
- Rust uses local `harper-core` from `src-tauri/Cargo.toml`: `harper-core = { path = "../harper/harper-core" }`.
...
- The root `harper/` directory is generated by `just pull-dep-source`; do not treat it as hand-maintained project code.
- `just clean` removes root `harper/`.
- `just pull-dep-source` currently switches the Harper checkout to `master`.
- CI installs `wasm-pack`, `cargo-xwin`, `tauri-cli`, and `cargo-hack`, then runs the relevant `just build-*` task.
