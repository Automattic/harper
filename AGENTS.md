...
6. `./AGENT_POLICY.md`: Harper's policy on code generated by large language models. Agents MUST read this before making ANY changes.
...
Harper's non-English languages (German, Portuguese, Slovak, Polish) live in `harper-core/src/language/`, behind per-language Cargo features. If you are touching anything language-related, read `harper-core/src/language/AGENTS.md` first — it covers the module architecture, which files are generated by `build.rs`, and the rule that language-specific code must not leak into shared core.
